...
float *loadCSV( const char *filename ) {
	...
	for( int i = 0; i < N*n_original_dims; i+=n_original_dims ) {
		...
		shuffle_idx = i + ( rand() % (N-(i/n_original_dims)) )*n_original_dims;
	...
	if( iteration == stop_iteration ) {
		...
		for( int i = 0; i < size; i++ ) {
			...
			for( int j = 0; j < V_SET_SIZE; j++ ) {
				...
				g_idx[i*(V_SET_SIZE+S_SET_SIZE) + j ].index = rand()%(g_interpolating?fixedsize:size);
			}
		}
	}
//for (int a = 0; a < (V_SET_SIZE+S_SET_SIZE)*N; a++) {printf("%d\n",g_idx[a].index); }
	...
	for( int i = fixedsize; i < size; i++ ) {
		...
		for( int j = 0; j < V_SET_SIZE+S_SET_SIZE; j++ ) {
			...
			g_idx[i*(V_SET_SIZE+S_SET_SIZE)+j].highd=(float) distmat[i*N+idx];     //i*N-akk+idx
			//printf("init embed\n");
			//printf("%d\n",akk);
			//printf("%d\n",i*N-akk+idx);
		...
		*/
		...
		// move the point
		for( int j = 0; j < (V_SET_SIZE+S_SET_SIZE); j++ ) {
//for (int a; a < N*(V_SET_SIZE+S_SET_SIZE); a++) { printf("%d\n",g_idx[a].index); } break;
...
//printf("init\n"); printf("%d ",idx*2+k); printf("%d ",i*(V_SET_SIZE+S_SET_SIZE)+j); printf("%d ",g_idx[i*(V_SET_SIZE+S_SET_SIZE)+j].index); printf("%d\n",k);
				// calculate the direction vector
				dir_vec[k] =  g_embed[2*idx*n_embedding_dims+k] - g_embed[i*n_embedding_dims+k];
//printf("%f ",dir_vec[k]); printf("%f ",g_embed[idx*n_embedding_dims+k]); printf("%f\n",g_embed[i*n_embedding_dims+k]); 
